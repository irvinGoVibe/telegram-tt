...
function unsafeMigrateCache(cached: GlobalState, initialState: GlobalState) {
  ...
  const legacySharedSettings = cachedSharedSettings as typeof cachedSharedSettings & {
    chatFolderLayout?: 'tabs' | 'sidebar';
  };
  if (legacySharedSettings.chatFolderLayout) {
    cachedSharedSettings.foldersPosition = legacySharedSettings.chatFolderLayout === 'sidebar'
      ? FOLDERS_POSITION_LEFT
      : FOLDERS_POSITION_TOP;
    delete legacySharedSettings.chatFolderLayout;
  }
